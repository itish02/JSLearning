...
dogsJulia.forEach(function (age) {
  age > 3
    ? console.log(`The dog is ${age} years old`)
    : console.log('The dog is still a puppy');
});

const checkDogs = function (dogsJ, dogsK) {
  const dogsJCopy = dogsJ;
  dogsJCopy.splice(0, 1);
  dogsJCopy.splice(-2);
  console.log(dogsJCopy);
  const allDogs = [...dogsJCopy, ...dogsK];
...
//const depositsFor = []; (Does the same thing as filter)
...
const calcAverageHumanAge = function (ages) {
  ...
  const calcAverage = humanAges.reduce(
    (acc, cur) => acc + cur / humanAges.length,
    0
  );
  console.log(humanAges, exclude);
...
for (let acc of accounts) {
  if (acc.username === 'stw') console.log(acc);
}
...
const accTotal = allMov.reduce((acc, mov) =>  acc + mov, 0)
console.log(accTotal);


// we can do this with chaining // flat
...
console.log(convertTitleCase('this is a Very LONG stRing But MehOOF'));
*/
...
const recommendedFood = function(dog) {
   dog.forEach((pup) => {
     pup.recommendedFood = Math.round(pup.weight ** 0.75 * 28);
  });
  return dog;
}
const recommended =  recommendedFood(dogs);
...
console.log(sarah);
console.log(`Sarah's dog is eating too ${sarah.curFood > sarah.recommendedFood ? 'much!' : 'little!'}`);
//if (sarah.curFood > sarah.weight ** 0.75 * 28) console.log('Sarah\'s dog is eating too much!');
...
// 4.
const string1 = ownersEatTooMuch.join(' and ') + "'s dogs eat too much!";
const string2 = ownersEatTooLittle.join(' and ') + "'s dogs eat too little!";
console.log(string1);
console.log(string2);

// 5.
const exact = dogs.includes(dog => dog.curFood === dog.recommendedFood);
console.log(exact);

// 6.
const eatingOkay = dog => dog.curFood > (dog.recommendedFood * 0.90 ) && dog.curFood < (dog.recommendedFood * 1.10)

const okayAmt = dogs.some(eatingOkay);
console.log(okayAmt);

// 7.
const okay = dogs.filter(eatingOkay)
console.log(okay);
